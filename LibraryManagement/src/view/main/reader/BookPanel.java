...
public class BookPanel extends javax.swing.JPanel {
    ...
    private ArrayList<String> columnTitlesOfJTable_DSSach = new ArrayList<>(Arrays.asList("ID", "title", "publish_date", "price", "location", "author", "publisher", "category", "rate"));

    public BookPanel() {
        initComponents();
        getAndShowData(jTable_Book, columnTitlesOfJTable_DSSach, "SELECT S.book_id as ID, S.title, S.publish_date, S.price ,L.location, TG.name as author, NXB.name as publisher, TL.category as category , FB.rate FROM book S, author TG, publisher NXB, category TL , location L, feedback FB WHERE S.author_id = TG.author_id AND S.category_id = TL.category_id AND S.publisher_id = NXB.publisher_id AND S.location_id = L.location_id  AND S.book_id = FB.book_id ");
